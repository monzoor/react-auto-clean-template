...
const updateCRAPackageJson_1 = __importDefault(
  require('./initialization/updateCRAPackageJson'),
);
const updateIndex_1 = __importDefault(require('./initialization/updateIndex'));
const copyTemplate_1 = __importDefault(
  require('./initialization/copyTemplate'),
);
const templateProcessor_1 = __importDefault(
  require('./initialization/templateProcessor'),
);
...
const init = async () => {
  ...
  const cra = await child_process_1.spawn(
    'npx',
    ['create-react-app', 'new', '-y'],
    {
      stdio: 'inherit',
  ...
  cra.on('exit', async () => {
    ...
    console.log(
      chalk_1.default.blue('Fetching clean architecture template...'),
    );
    await deleteFiles_1
      .default(fileNames_1.default.DELETE_LIST)
      .then((response) => {
    ...
    fileNames_1.default.COPY_LOCATIONS.forEach(async (location) => {
      await copyTemplate_1
        .default(location.src, location.dest)
        .then((response) => {
          console.log(response);
        });
    });
    await deleteFiles_1
      .default(fileNames_1.default.CLEAN_UP)
      .then(async (response) => {
        console.log(response);
